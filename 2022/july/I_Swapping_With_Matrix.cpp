...
#define MAX 1000005
#define mod 1000000007
// in matrix swap row a with row b and column a with column b
void swap_row(vector<vector<ll>> &mat, ll a, ll b)
{
    vector<ll> temp = mat[a];
    mat[a] = mat[b];
    mat[b] = temp;
}

void swap_col(vector<vector<ll>> &mat, ll a, ll b)
{
...
int32_t main()
{
    ...
    swap_row(mat, a - 1, b - 1);
    swap_col(mat, a - 1, b - 1);
    for (ll i = 0; i < n; i++)
    {
        for (ll j = 0; j < n; j++)
        {
