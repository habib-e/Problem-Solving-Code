...
int32_t main()
{
    ...
    for (ll i = 1; i <= t; i++)
    {
        ...
        while (m--)
        {
            ...
            string s;
            cin >> s;
            if ((s == "pushLeft" || s == "pushRight") && (dq.size() >= n)) // checking if the double ended queue is full.
            {
                cin >> x;
                cout << "The queue is full\n";
            }
            else if ((s == "popLeft" || s == "popRight") && dq.size() == 0) // checking if the double ended queue is empty.
            {
